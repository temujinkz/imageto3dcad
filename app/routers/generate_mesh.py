...
from .upload import _save_upload_as_png, _process_job


def build_router(store: JobStore, settings: Settings) -> APIRouter:
    router = APIRouter(tags=["mesh"])

    @router.post("/generate-mesh", response_model=JobAcceptedResponse, status_code=202)
