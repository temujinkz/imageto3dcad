...
// Pull the most useful reason out of the backend warnings so a low-fidelity
// result explains itself (e.g. cloud credits ran out) instead of a vague badge.
function qualityNote(warnings?: string[]): string | null {
  ...
  if (credit) {
    return "The cloud 3D provider is out of credits, so this is the offline engine's colored estimate. Top up WaveSpeed (or add another provider key) for a photoreal, fully textured mesh.";
  }
  const local = warnings.find((w) => /silhouette|offline|no 3D API|stylized/i.test(w));
  if (local) {
    return "Generated by the offline engine (no cloud 3D provider configured). Colors come from your photo; add a provider key for photoreal geometry.";
  }
  return null;
}
...
export function ModelViewer({ modelUrl, meshSource, meshIsHighFidelity, warnings, busy, busyLabel }: ModelViewerProps) {
    ...
    <section className="rounded-card border border-line bg-card p-4 shadow-card sm:p-5">
      ...
      {canPreview && meshSource && (
        ...
        >
          ...
          {meshIsHighFidelity
            ...
            : "Offline colored estimate"}
        </div>
      )}

      {canPreview && note && (
        <p className="mb-3 rounded-[12px] border border-accent/20 bg-accent/5 px-3.5 py-2.5 text-xs leading-5 text-ink">
          {note}
        </p>
      )}
      ...
      <div className="h-[480px] overflow-hidden rounded-[14px] border border-line bg-bone">
        {canPreview && modelUrl ? (
          <Canvas
            key={resetKey}
            camera={{ position: [2.8, 2.2, 3.4], fov: 42 }}
            dpr={[1, 1.75]}
